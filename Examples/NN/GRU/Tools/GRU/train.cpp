...
void read_x_y(MatrixXf& x, MatrixXf& y, std::string filename_input, std::string filename_output, int time_steps, int pos) {
    ...
    while(!file_input.eof() && count < time_steps) {
    ...
    while(!file_output.eof() && count < time_steps) {
...
int train(std::string filename_input, std::string filename_output, float learning_rate, int nepoch, int input_dim, int hidden_dim, int output_dim, int time_steps, float decay) {
...
int main(int argc, char *argv[])
{
    ...
    int status = train("Inputs/donald_trump_input.txt", "Inputs/donald_trump_output.txt", learning_rate, nepochs, input_dim, hidden_dim, output_dim, time_steps, decay);
    return 0;
}
