...
//  V3D_INT_QPU_MASK    = V3D_MASK(27, 16),
  ...
  //
  // NOTE: sequences CORE_PCTR_0_SRC_[0-7] and CORE_PCTR_0_PCTR[0-31] made public for performance counters
  //
...
#define __wait_for(OP, COND, US, Wmin, Wmax) ({ \
  ...
  unsigned wait__ = (Wmin); /* recommended min for usleep is 10 us */ \
  int ret__;              \
  might_sleep();              \
  for (;;) {              \
